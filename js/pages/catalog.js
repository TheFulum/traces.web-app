...
function wireTripButtons() {
  grid.querySelectorAll('[data-trip-toggle]').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      const id = btn.getAttribute('data-trip-toggle');
      const name = btn.getAttribute('data-trip-name') || '';
      if (!id) return;
      if (isInTrip(id)) {
        removeFromTrip(id);
        btn.textContent = t('trip.add', dictI18n);
        showToast(t('trip.removedToast', dictI18n));
      } else {
        addToTrip({ id, name });
        btn.textContent = t('trip.remove', dictI18n);
...
function placeCard(p, idx) {
  ...
  const inTrip = isInTrip(p.id);
  const tripLabel = inTrip ? t('trip.remove', dictI18n) : t('trip.add', dictI18n);
    ...
    <article class="card reveal reveal--stagger" style="--i:${idx % 8}" onclick="location.href='place.html?id=${escAttr(p.id)}'">
      ...
      <div class="card__body">
        ...
        <div class="card__actions">
          <button type="button" class="btn btn--outline btn--sm" data-trip-toggle="${escAttr(p.id)}" data-trip-name="${escAttr(name)}">
            ${esc(tripLabel)}
          </button>
