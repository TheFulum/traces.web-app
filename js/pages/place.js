...
import { getPlace } from '../shared/places.js';
import { showToast, getParam, checkRateLimit, formatRemaining } from '../shared/utils.js';
import { addPlaceReview, getPlaceReviews } from '../features/placeReviews.js';
...
import { formatPlaceDate } from '../shared/placeDate.js';
import { addToTrip, removeFromTrip, isInTrip } from './trips.js';
...
function render(place) {
  ...
  // trip button
  initTripButton(place.id, placeName);
...
function initTripButton(placeId, placeName) {
  const btn = document.getElementById('btn-trip');
  const label = document.getElementById('btn-trip-label');
  if (!btn || !label) return;

  const refresh = () => {
    label.textContent = isInTrip(placeId) ? t('trip.remove', dictI18n) : t('trip.add', dictI18n);
  };
  refresh();
  ...
  btn.addEventListener('click', () => {
    if (isInTrip(placeId)) {
      ...
      showToast(t('trip.removedToast', dictI18n));
    } else {
      addToTrip({ id: placeId, name: placeName });
  ...
  async function loadReviews(reset) {
    ...
    try {
      if (reset) {
        ...
        // update stats
        const countEl = document.getElementById('reviews-stats-count');
        const avgEl = document.getElementById('reviews-stats-avg');
        if (countEl && avgEl) {
          countEl.textContent = allReviews.length;
          if (allReviews.length > 0) {
            const sum = allReviews.reduce((acc, r) => acc + (r.rating || 0), 0);
            avgEl.textContent = (sum / allReviews.length).toFixed(1);
          } else {
            avgEl.textContent = '—';
          }
        }
