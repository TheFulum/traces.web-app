...
function renderTable() {
  ...
  tbody.innerHTML = places.map(p => {
    ...
    if (p.modelUrl && p.modelType !== 'sketchfab') {
      ...
      // Legacy: modelUrl without modelType — check if it's a sketchfab URL
...
function buildSketchfabEmbed(url) {
  ...
  // https://sketchfab.com/3d-models/church-name-a829ad5bfa7a4b8c995b54a4708f6362
  // The model ID is the last 32 hex characters of the slug
  ...
  // Try direct model ID format: /models/abc123/embed
...
function openModal(place) {
  ...
  if (place?.sketchfabUrl) {
    ...
    // Legacy: modelUrl was a sketchfab link
  ...
  // If we have a sketchfab URL, show preview
...
async function save() {
  ...
  // Validate sketchfab URL if that mode is selected
  ...
  try {
    ...
    // Determine final model data based on selected type
